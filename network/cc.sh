...
docker exec cli peer chaincode upgrade -n genedata -v 1.0 -C mychannel -c '{"Args":["init"]}' -P 'OR ("Org1MSP.member", "Org2MSP.member")'
...
docker exec cli peer chaincode invoke -n genedata -C mychannel -c '{"Args":["addUser","a"]}'
#chaincode invoke b
docker exec cli peer chaincode query -n genedata -C mychannel -c '{"Args":["getUser","a"]}'
...
sleep 5
#chaincode query b
docker exec cli peer chaincode query -n genedata -C mychannel -c '{"Args":["getGene","4"]}'
