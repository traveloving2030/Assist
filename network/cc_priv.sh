...
docker exec cli peer chaincode instantiate -n PrivateGene -v 1.0 -C mychannel -c '{"Args":["init"]}' -P 'OR ("Org1MSP.member", "Org2MSP.member")' --collections-config  /opt/gopath/src/github.com/PriveateGene/collections_config.json 
docker exec cli peer chaincode upgrade -n PrivateGene -v 1.2 -p -C mychannel github.com/PriveateGene
docker exec cli peer chaincode upgrade -n mycc -v 1 -c '{"Args":["d", "e", "f"]}' -C mychannel
docker exec cli peer chaincode install -n PrivateGene -v 1.1 -p github.com/PriveateGene
docker exec cli peer chaincode upgrade -n PrivateGene -v 1.2 -C mychannel -c '{"Args":["init"]}' -P 'OR ("Org1MSP.member", "Org2MSP.member")' --collections-config  /opt/gopath/src/github.com/PriveateGene/collections_config.json 
...
docker exec cli peer chaincode invoke -o orderer.example.com:7050 -n PrivateGene -C mychannel -c '{"Args":["saveskininfo"]}'  --transient "{\"putPrivateGene\":\"$MARBLE\"}"
docker exec cli peer chaincode query -C mychannel -n PrivateGene -c '{"Args":["getskininfo","3"]}'
